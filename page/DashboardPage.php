...
class DashboardPage
{
    ...
    #[Pure] public static function getDayText(): string
    {
        if (is_int(self::$kwh)) {
            $watt = self::$kwh * 1000;
            if ($watt < 50) {
                return "Hiermee kunt u een 8 watt ledlamp wel " . number_format($watt / 8) . " uur laten branden!";
            } elseif ($watt > 50 && $watt <= 200) {
                return "Hiermee kunt u wel gemiddeld " . number_format($watt / 45) . " uur tv kijken!";
            } elseif ($watt > 200 && $watt <= 400) {
                return "Hiermee kunt uw smartphone wel gemiddeld " . number_format($watt / 20) . " keer opladen!";
            } elseif ($watt > 400 && $watt <= 800) {
                return "Hiermee kunt u wel " . number_format($watt / 65) . " keer uw laptop opladen!";
            } elseif ($watt > 800 && $watt <= 2000) {
                return "Uw kunt hiermee wel " . number_format(($watt / 800) * 12) . " keer uw eten voor 5 minuten in de magnetron opwarmen!";
            } elseif ($watt > 2000 && $watt <= 4000) {
                return "Hiermee kunt u wel " . number_format(($watt / 900) * 4) . " keer voor een kwartier uw huis stofzuigen!";
            } else {
                return "U kunt hiermee wel " . number_format($watt / 1500) . " keer voor een uur lang koken op een elektrischekookplaat";
            }
        } else {
            return "Er is vandaag nog niets opgewekt.";
        }
    }
    ...
    #[Pure] public static function calcWeekEuroTotal(): string
    {
        ...
        if (!empty(self::$weekDateArr)) {
            $sum = 0;
            for ($i = 0; $i < count(self::$weekDateArr); $i++) {
                if (isset(self::$weekEuroArr[$i])) {
                    // Add item in array to sum
                    $sum += self::$weekEuroArr[$i];
                } else {
                    continue;
                }
            }
